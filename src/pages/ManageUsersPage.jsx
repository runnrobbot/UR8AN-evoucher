...
import { Users, Search, ShieldCheck, Shield, User, Plus, Eye, EyeOff } from "lucide-react";
import { createUserWithEmailAndPassword, updateProfile } from "firebase/auth";
import { doc, setDoc, serverTimestamp } from "firebase/firestore";
import { secondaryAuth, db } from "../lib/firebase";
import { useAuth } from "../contexts/AuthContext";
import { useUsers, updateUserRole } from "../hooks/useUsers";
...
const emptyCreate = { displayName: "", email: "", password: "", role: "user" };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", cred.user.uid), {
        uid: cred.user.uid, email: createForm.email,
        displayName: createForm.displayName.trim(), role: createForm.role,
        photoURL: null, createdAt: serverTimestamp(), createdBy: currentUser?.uid,
      });
      await secondaryAuth.signOut();
      setCreateOpen(false); setCreateForm(emptyCreate);
    } catch (err) {
              ...
              <table className="w-full">
                ...
                <motion.tbody variants={staggerContainer} initial="hidden" animate="visible" className="divide-y divide-slate-50">
                  {filtered.map((u) => {
                    ...
                    return (
                      <motion.tr key={u.id} variants={tableRow} className="hover:bg-slate-50/50">
              ...
              {filtered.map((u) => {
                ...
                return (
                  <motion.div key={u.id} variants={staggerItem} className="px-4 py-4 flex items-center justify-between gap-3">
                    ...
                    {!isSelf && (
                      <button onClick={() => openRoleModal(u)} className="text-xs text-[#6B8F71] hover:underline font-medium shrink-0">Edit</button>
                    )}
                  </motion.div>
                );
              })}
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              <input type={showPass ? "text" : "password"} placeholder="Min. 6 karakter"
                ...
                className={`w-full rounded-xl border px-4 py-2.5 text-sm outline-none pr-10 focus:ring-2 focus:ring-slate-400 focus:border-slate-500
                  ${createErrors.password ? "border-red-400 bg-red-50" : "border-slate-200"}`}
              />
      ...
      <Modal open={!!roleModal} onClose={() => setRoleModal(null)} title="Ubah Role" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            Mengubah role untuk <span className="font-semibold text-[#2D3A3A]">{roleModal?.name}</span>
          </p>
